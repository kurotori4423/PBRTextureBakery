...
class PBR_CreateTextrue(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        col = self.BakeTextureMultiMat(context.scene.bakery_filename, ao, 'Base Color', context.scene.bakery_resolution)
        if col is None:
            return{'FINISHED'}
        saveTexture(context, col, 'PNG', col.name, context.scene.bakery_out_directory)

        met = self.BakeTextureMultiMat(context.scene.bakery_filename, ao, 'Metallic', context.scene.bakery_resolution)
        if met is None:
            return{'FINISHED'}
        saveTexture(context, met, 'PNG', met.name, context.scene.bakery_out_directory)

        nrm = self.BakeNormalMultiMat(context.scene.bakery_filename, ao, context.scene.bakery_resolution)
        if nrm is None:
            return{'FINISHED'}
        saveTexture(context, nrm, 'PNG', nrm.name, context.scene.bakery_out_directory)

        rgh = self.BakeTextureMultiMat(context.scene.bakery_filename, ao, 'Roughness', context.scene.bakery_resolution)
        if rgh is None:
            return{'FINISHED'}
    ...
    def CheckNextMixNode(self, mixnode):
        ...
        return result
    def BakeNormalMultiMat(self, filename, activeObject, resolution):
    ...
    def BakeTextureMultiMat(self, filename, activeObject, mapType, resolution):
